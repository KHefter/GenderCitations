...
get.cr.sep=function(x){
  if(!is.null(x$given)){
    ...
    return(name)
  }# KAT ADDED TO ACCOUNT FOR WEIRD CASE
  else if(is.null(x$given)&is.null(x$family)){
    return(x$name)
  }
...
citeprops=function(ref_proportions,i=NULL,type='conditional'){
  ...
  if(type=='conditional'){
    ...
    i=i[!nas]
    out=round(matrix(apply(ref_proportions[i,c(1:4,9:12)],2,sum)/
                       sum(apply(ref_proportions[i,1:4],2,sum)),
                     byrow=T,ncol=4),3)
    rownames(out)=c("Observed props.","Expected props.")
  }else if(type=='randomdraw'){
    nas=apply(is.na(ref_proportions[i,c(1:12)]),1,sum)!=0
    i=i[!nas]
    out=round(matrix(apply(ref_proportions[i,c(1:4,5:8)],2,sum)/
                       sum(apply(ref_proportions[i,1:4],2,sum)),
                     byrow=T,ncol=4),3)
    rownames(out)=c("Observed props.","Expected props.")
  }else{
